...
namespace Ironclad
{
    ...
    public partial class PythonMapper: PythonApi
    {
        ...
        public override IntPtr PyMemoryView_FromObject(IntPtr obj)
        {
            var o = Retrieve(obj);
            if (o is MemoryView mv) {
                return Store(new MemoryView(mv));
            }
            if(o is IBufferProtocol bp) {
                return Store(new MemoryView(bp));
            }

            // TODO: type name
            LastException = PythonOps.TypeError("memoryview: object does not have the buffer interface");
            return IntPtr.Zero;
        }

        public override string Py_GetVersion()
        {
            return this.python.SystemState.Get__dict__()["version"] as string;
        }


        public override int PyErr_WarnEx(IntPtr category, string message, nint stack_level)
        {
            if(category == IntPtr.Zero)
            {
                PythonOps.Warn(this.scratchContext, Builtin.RuntimeWarning, message);
            }
            else
            {
                PythonOps.Warn(this.scratchContext, (PythonType)Retrieve(category), message);
            }
            // TODO: stack_level?
            return 0;
        }
